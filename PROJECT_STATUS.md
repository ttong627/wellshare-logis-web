...
- 🟢 **(2026-09-13 형 「모두 고」) 코코 보안 지적 H2·H3·M2 처리**
  ...
  - **H2 인증된 이메일만 관리자**:
    ...
    - 🔁**코난 재점검(규칙 배포 뒤)으로 H2 재개 — 우회로 1곳 + 오류 표시 1곳**
      ...
      - ⑦(코난 마무리 검증 · Codex 한도 초과·셜록 막힘으로 **Claude 쪽 실측만**) ⑥의 메시지 판정이 `includes` 라 **서명 없는 위조 토큰**의 aud·iss·alg 에 `Error fetching public keys` 를 넣으면 502(인증 서버 장애)로 위장됐다(서명 검사 전 내용 검사가 공격자 값을 메시지 중간에 옮김). 인증 우회는 아님. 고침: `startsWith`(진짜 조회 실패 문구는 항상 맨 앞). 실제 firebase-admin 메시지로 도는 시험 `ecount-gateway/test/firebase-admin-messages.test.mjs`(위조 3·조회 실패 2, 네트워크 없이) 추가 — 전체 18건, `includes` 로 되돌리면 2건 실패(Red-Green). 게이트웨이 리비전 **00008-hsq**. 제시 통과(연결 거부·HTTP 500 은 502 유지 실측)
      ...
      - ✅**(2026-09-14 배포) functions 런타임 Node 20 → 22** — 커밋 `95724da`·`aa8d7ce`(engines·firebase.json·lock). 배포 전후 설정 비교: 3개 모두 **런타임 말고 달라진 것 없음**(실행 계정·시크릿·트리거·메모리·ingress 동일) · nodejs22 ACTIVE · 리비전 `adminsetpassword-00003-guj`·`sendpushonnotification-00003-vaq`·`rosteralert-00002-fil`(이전 `-00002-cor`·`-00002-lep`·`-00001-tax`) · 로그인 없이 호출(POST `{"data":{}}`) **401 「로그인이 필요합니다」** · 배포 뒤 10분 ERROR 0. ↩️되돌리기: `gcloud run services update-traffic <서비스> --to-revisions=<이전 리비전>=100 --region asia-northeast3 --project wellshare-logis`
      - 확인: `users` 컬렉션 문서 0개 → 규칙의 `ROLE_ADMIN`·`ROLE_EDITOR` 판정은 쓰이지 않는 옛 흔적(계정 없음) · ✅(2026-09-14) 셀프 승격 틈 2곳 막음 — 아래 「남는 위험」 users 항목
      - ✅**(2026-09-14) 관리자 행 「비번설정」 버튼 숨김** — 정산 앱 v2.19.3(`e2931fa`·`4ab3e5e` sw.js 캐시명 — `npm version` 은 sw.js 를 안 올려 CI 버전 검사에 걸릴 뻔함(제시) · CI run 34766975505 success · 운영 sw.js `wellshare-pwa-v2.19.3` · 200 · ↩️이전 hosting 버전 `2c201d8052c60c36`) · wslos.kr 1.22.2(wellshare-platform `a9ced13`, main 먼저 푸시 후 배포 · 운영 version.json 1.22.2 확인). `company==='ADMIN'` 행만 숨김 — 운영 partnerAccounts 9개(ADMIN 3·회원사 6) 기준 버튼이 남는 보호 계정 0(제시). 게이트웨이 관리자가 회원사 값으로 바뀌면 버튼이 다시 보이지만 서버가 거부
  - ⚠️**남는 위험**: 회원사 6곳 전부 미인증 + 자가가입 화면 → 앞으로 추가될 회원사 이메일은 먼저 가입해 선점 가능(회원사 인증 메일 흐름 필요) · wslos.kr SSO 는 통합계정 이메일 일치로 정산 토큰 발급(신뢰가 wslos 이메일 관리에 걸림) · `wssc-admin` MFA 없음·키 평문 · 코코 M1(서버키 권한 범위·PHP 로그인만 하면 발행)·M3(S3 업로더 GetObject)·LOW 는 이번 범위 밖 · 운영 `ALLOWED_ORIGINS` 에 `http://localhost:5173` 남음 · ECOUNT 키 시크릿이 `:latest`(버전 미고정) · `/ecount/sale-tms` 는 TMS 폐기로 항상 500(죽은 경로, 코드 정리 필요) · (코난 2026-09-13) **기본 compute SA 가 아직 프로젝트 Editor**(Cloud Functions 가 이 계정으로 돈다 — 게이트웨이만 전용 SA 로 뺐다) · 인증 메일 보내는 흐름(`sendEmailVerification`) 없음 → 새 동적 관리자는 콘솔에서 인증 처리해야 관리자 권한이 생긴다 · ✅**(2026-09-14 배포) `users` 셀프 승격 틈 2곳 막음** — ①create 가 role 값을 안 막음 → `!('role' in incoming())` ②users 아래 `match /{allSubcollections=**}` 가 rules v2 에서 **경로 0개에도 맞아** `users/{uid}` 문서 자체 write 를 열었음(create·update 검사 우회 — 시뮬레이터 visitedExpressions 로 확인) → `match /{subcollection}/{subDoc=**}`. 커밋 `91ebd84` · `scripts/rules-test.py` users 시험 8건(RED 44/47 → GREEN 49/49 · 제시 Red-Green: 한쪽만 되돌리면 47/49·46/49) · 운영 ruleset **`35754a9c-636a-421c-88a2-7352192499c7`**(`releases/cloud.firestore` 연결 확인 · 운영본 시뮬레이터 49/49 · 배포 전후 users 문서 0) · ↩️이전 `88cef7e3-7f85-4c96-b47e-459d2e07af85` · 코난 조건부(Codex 한도 초과 → 셜록 이어받음) 3가지(배포 순서·함수 설정 전후 비교·운영 연결 확인) 반영 뒤 배포 · ✏️코난 정정(09-14): git 에 없는 옛 앱 `ttong/`(`firebaseService.ts:178-190`)이 첫 로그인 때 users 문서를 role 넣어 만든다 — 이 앱 대상은 철거된 `gen-lang-client-0075547354` 라 지금 영향 없음. **옛 앱을 복구해 wellshare-logis 에 다시 붙이면 새 규칙에 가입이 막힌다**(users·invites 블록은 그 앱 규칙을 복사해 온 것 · 같은 `=**` 틈이 원본에도 있었음) · ⚠️**(코난 09-14 발견 · 기존 틈 · 이번 배포와 무관)** 최상위 `settings`·`officialDocs`·`drivers`·`schedules` 는 로그인만 하면 쓰기·삭제 가능(`firestore.rules:204-249`) — 셀프가입이 열려 있어 누구나 로그인 가능. 이 저장소 코드는 전부 `artifacts/…` 경로라 안 씀 → **안 쓰는 최상위 규칙 정리 대상** · `users` 는 로그인만 하면 읽기(문서 0이라 지금 샐 것 없음) · `invites` update 로그인만(비었음·쓰는 코드 없음) · 로그인만 하면 쓸 수 있는 `settings`·`drivers`·`officialDocs`(코난 판단 Low) · 관리자 목록을 네 곳에서 손으로 맞춤(`firestore.rules`·`storage.rules`·functions `ADMIN_EMAILS`·게이트웨이 env ↔ functions `GATEWAY_ADMIN_EMAILS`) — 어긋나도 테스트가 못 잡음
...
- 🟢 **(2026-09-13 14:01 KST 삭제 완료) logis-TMS(`gen-lang-client-0075547354`) — 옮길 것 옮기고 보관한 뒤 철거. 복구 기한 2026-10-13**
  ...
  - ⏸️**남은 것**: ①~~wslos 운영 DB 감시 행 3개(`gen_lang`·`wellshare_tms`·`wellshare_erp`)~~ → ✅2026-09-13 21:01 삭제(아래 ⑥, 00156-v9t 배포 뒤 `--apply`) ②모바일 앱 새 주소 빌드(`a8ba1e1` 반영) ③~~`ecount-gateway/README.md` 옛 프로젝트 배포 명령~~ → 2026-09-13 새 프로젝트·전용 SA 기준으로 다시 씀 ④옛 호스팅 두 곳이 삭제 직후 아직 200 — 며칠 뒤 404 확인 ⑤S3 로 바꾼 백업의 **첫 정기 실행 = 2026-09-14 02:00 KST** → 다음 날 `backup.log` 「local + S3」 확인 · `archive_images.sh` 실제 S3 이관은 395일 넘는 사진이 생기는 2026-12 이후 첫 실행(같은 cp+ls 확인 절차는 임시 파일로 성공 확인) ⑥**(2026-09-13 발견) 철거 누락 — wslos.kr 서버 쪽 genlang 잔재**: 화면 genlang 82파일만 지우고 서버를 놓쳤다. `wellshare-platform/backend/app/core/firebase_minters.py:18`(`firebase_genlang`→`FIREBASE_SA_GENLANG`)·`providers.py:6`·`models/identity.py:64`·테스트 2곳, 운영 Cloud Run `wellshare-platform-api` env `FIREBASE_SA_GENLANG` → Secret Manager **`wp-sa-genlang`**(`:latest`, Cloud Run Job 참조 없음 — 제시 보충으로 실제 이름 확인), `directed-line-434014-h0` Firestore 복제 DB `genlang`. 프로젝트가 없어 기능은 죽어 있음 → ✅**2026-09-13 21시 처리**(형: 「AWS 서버로 다 이전했다」 — 묻지 말고 치울 것): 플랫폼 main `b157a8e`(provider·minter·시험 제거, 워크트리에서 빌드해 다른 세션 미커밋 제외) · 전체 pytest 1501 passed · 운영 `wellshare-platform-api` **00156-v9t**(이미지 digest `f161a5e3…`, `--remove-secrets=FIREBASE_SA_GENLANG`, env 24→23) · Firestore 복제 DB `genlang` 삭제(원본 최종 내보내기는 logis-TMS 보관본) · 운영 DB 잔재 삭제(`scripts/cleanup_genlang_remnants.py --apply` via `wp-adhoc`: mon_targets 3·mon_checks 31,323·mon_incidents 1·linked_accounts 1 → 남은 0, 삭제 전 CSV `gs://wellshare-platform-db-backup/genlang-cleanup-20260913/` — 백업 뒤 쌓인 점검 15행은 백업 없음) · 시크릿 `wp-sa-genlang` 은 새 리비전 12분 관찰(요청 12·5xx 0·ERROR 0·헬스 200) 뒤 **21:10 삭제**(롤백은 옛 digest 재배포 + `--remove-secrets` 로 가능)

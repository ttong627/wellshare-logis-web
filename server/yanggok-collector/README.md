...
- **수집 상태(하트비트)**: COMMIT 실행 후 `settings/yanggok_status` 단일 문서를 통째로 덮어써
  마지막 실행 시각·신규 건수·계정 정상 여부·실패 요약을 기록한다. 앱 「명단 탭」이 이를 구독해
  **관리자에게만** 수집 정상/지연/실패 배너로 보여준다(5시간 넘게 갱신 없으면 "지연"). dry-run은 미기록.
